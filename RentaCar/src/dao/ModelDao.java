...
public class ModelDao {
    ...
    public boolean save(Model model) {
        String query = "INSERT INTO model " +
                ...
                "model_gear," +
                ")" +
                "VALUES (?,?,?,?,?,?)"; // entitydeki get mototları ile dolduruldu
        try {
            PreparedStatement pr = con.prepareStatement(query);
    ...
    public Model match(ResultSet rs) throws SQLException {
        Model model = new Model(); // db den gelen her rs satırı burada entity verisine dönüşüyor
        model.setId(rs.getInt("model_id")); // entity deki ID ye denk geliyor
        model.setName(rs.getString("model_name")); // entity deki model_name denk geliyor vb...
